...
import { useLazyGetSummaryQuery, useGetPdfSummaryMutation } from "../services/article";
...
const Demo = () => {
  ...
  const [getPdfSummary, { isLoading: isPdfLoading }] = useGetPdfSummaryMutation();

  const handlePdfParsed = async (parsedData) => {
    const { data } = await getPdfSummary(parsedData).unwrap();
    if (data?.choices?.[0]?.text) {
        const newArticle = { url: 'PDF Document', summary: data.choices[0].text };
  ...
  return (
    <section className="mt-16 w-full max-w-xl">
      ...
      <div className="flex flex-col w-full gap-2">
        <form className="relative flex justfiy-center items-center" onSubmit={handleSubmit}>
